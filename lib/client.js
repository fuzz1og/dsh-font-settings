...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/** Probe text: wide/narrow/digit/CJK glyphs so unlike fonts diverge. */
		const PROBE_TEXT = "Wm1il|中文Aa";
		/** Generic sentinels; a visible family must diverge from at least one. */
		...
		function familyMatches(family) {
			...
			try {
				if (typeof document !== "undefined" && document !== null && document.body !== null) {
					...
					// can transiently resolve CJK fallback before the font cache is
