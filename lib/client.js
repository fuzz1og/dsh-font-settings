...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		 ...
		 */
		function buildTerminalAliasCss(codeStack, fonts) {
				...
				for (const alias of TERMINAL_ALIAS_FAMILIES) {
					for (const face of faces) {
						...
						rules.push("@font-face{font-family:" + alias
							...
							+ ";font-style:" + (face.italic === true ? "italic" : "normal") + "}");
					}
				}
			} else {
				const quoted = quoteLocal(family);
				if (quoted !== null) {
					rules.push("@font-face{font-family:ui-monospace;src:local(" + quoted + ");font-weight:400;font-style:normal}");
				}
			}
			return rules.join("");
		}
		...
		function apply(ctx) {
			...
			const applyTerminalAlias = (codeStack) => {
				...
				const text = buildTerminalAliasCss(codeStack, systemFonts);
