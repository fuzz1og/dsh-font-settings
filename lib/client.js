...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/** Verdicts cached per family for the browser process lifetime. */
		 ...
		 * back from the DomRenderer's own row styling — it writes
		 * `font-size: <n>px` straight from the Terminal options, so this is
		 * exactly the number `size-adjust` multiplies.
		...
		function terminalBasePx() {
			try {
				...
				if (rows !== null) {
					const px = Number.parseFloat(getComputedStyle(rows).fontSize);
					if (Number.isFinite(px) && px > 0) return px;
				}
			} catch (error) {
				/* no terminal mounted, or no layout yet — use the fallback */
			}
		 ...
		 */
		function FontSettingsRow({ t, useStore, setFont, setSize }) {
		...
		function apply(ctx) {
			...
			const injected = (actions) => {
				...
				push();
				return { setFont: save, setSize: saveSize };
			};
			adopt({});
			load();
			loadFonts();
