...
async function listSystemFonts() {
  ...
  if (process.platform === "darwin") {
    // macOS keeps fonts in fixed locations; the sfnt name-table reader above is
    // platform-neutral, so the same parse path serves them unchanged.
  ...
  for (const dir of dirs) {
    ...
    for (const file of files) {
      ...
      // macOS marks private/system-internal families with a leading dot
      // (".LastResort", ".Aqua Kana"); they are not selectable in any font UI.
      if (info.family.startsWith(".")) continue;
