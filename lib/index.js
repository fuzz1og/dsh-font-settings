...
/**
 * Schemastery is resolved lazily on first use rather than with a static import,
 * so that a missing copy degrades this plugin instead of killing the whole boot.
 *
 ...
 *
 * Resolving on first use keeps the module evaluation infallible. When the
 * package is unavailable the settings namespace is never registered, the routes
 * answer 503, and one warning names the cause: the plugin is inert, the harness
 * runs.
 */
let schemastery = null;
let schemasteryFailure = null;
let schemasteryLoading = null;
/**
 * Resolve schemastery once; concurrent callers share the attempt, and a failure
 * is remembered instead of being retried (the resolution cannot start working
 * without a reinstall).
 * @returns the resolved module (or null when it cannot be resolved).
 */
function loadSchemastery() {
  if (schemastery !== null || schemasteryFailure !== null) return Promise.resolve(schemastery);
  if (schemasteryLoading === null) {
    schemasteryLoading = import("@deepseek-ai/schemastery")
      .then((loaded) => {
        schemastery = loaded.default ?? loaded;
        return schemastery;
      })
      .catch((error) => {
        schemasteryFailure = error;
        return null;
      });
  }
  return schemasteryLoading;
}
 ...
 * @returns the schema, or null when schemastery could not be resolved (the
 *   plugin then mounts inert rather than aborting the profile).
 */
function fontSettingsSchema() {
  if (schemastery === null) return null;
  return schemastery.object({
    uiFont: schemastery.string().default("default"),
    uiFontValue: schemastery.string().default(""),
    codeFont: schemastery.string().default("default"),
    codeFontValue: schemastery.string().default(""),
    terminalSize: schemastery.number().default(0)
  });
}
...
export function apply(ctx) {
  ...
  ctx.inject(["settings"], (settingsCtx) => {
    ...
    // Registration waits on the lazy resolve; it settles in about a
    // millisecond on a real install, and the routes answer 503 until then.
    loadSchemastery().then(() => {
      if (detached) return;
      const schema = fontSettingsSchema();
      if (schema === null) {
        // Mount inert with the cause named, instead of having the loader abort
        // the whole profile over one unresolvable dependency.
        ctx.logger?.warn?.(
          "[dsh-font-settings] @deepseek-ai/schemastery could not be resolved, so the ui-font settings namespace was not registered and /font-settings will answer 503. "
          + "This is what a path-based install (`link:`) looks like when the linked checkout's own dependencies were never installed; reinstall from the published specifier. Cause: "
          + (schemasteryFailure instanceof Error ? schemasteryFailure.message : String(schemasteryFailure))
        );
        return;
      }
      ...
      scope = settingsCtx.settings.register(FONT_SETTINGS_NAMESPACE, schema);
    });
  });
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        if (req.method === "GET" && url.pathname === "/font-settings") {
          ...
          return sendJson(res, 200, {
            ok: true,
            value: { uiFont: value.uiFont, uiFontValue: value.uiFontValue, codeFont: value.codeFont, codeFontValue: value.codeFontValue, terminalSize: value.terminalSize }
          });
        }
 ...
 */
export { listSystemFonts, listSystemFontsRefreshed, readFontNames, sizeFor };
