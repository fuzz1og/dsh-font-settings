...
/**
 * Resolve schemastery from this package first, then from the running harness
 * install. The second base is what makes a `link:`/checkout install work: that
 * path's own `node_modules` cannot see `@deepseek-ai/schemastery` (it is a
 * dependency of the harness, not of this plugin), while the launcher's module
 * graph always carries it.
 * @returns the module namespace, or null when neither base resolves it.
 */
function resolveSchemastery() {
  ...
  for (const base of bases) {
    try {
      ...
      if (typeof resolved?.object === "function") return resolved;
    } catch (error) {
...
/**
 * The font preference schema — this plugin's `Config`, and therefore the
 ...
 *
 * `undefined` when schemastery could not be resolved: the entry then mounts
 * with no configurable fields and the routes answer 503, rather than the boot
 * being aborted over one unresolvable dependency.
 */
export const Config = schemastery === null ? undefined : schemastery.object({
  uiFont: schemastery.string().default("default").volatile(),
  uiFontValue: schemastery.string().default("").volatile(),
  codeFont: schemastery.string().default("default").volatile(),
  codeFontValue: schemastery.string().default("").volatile(),
  terminalSize: schemastery.number().default(0).volatile()
});
